...
def write_multi_join(canvas, t, scale = 1.0):
    sig = scale / abs(scale)
    x = sig * max(sig * ti.get_offset()[0] for ti in t) + 0.5 * scale
    y = sum(ti.get_offset()[1] for ti in t) / len(t)
    y_min = min(ti.get_offset()[1] for ti in t)
    y_max = max(ti.get_offset()[1] for ti in t)
    for ti in t:
        Line((ti.get_offset(), (x, ti.get_offset()[1]))).write(canvas)
    Line([
        (x, y),
        (x + 0.5 * scale, y)
    ]).write(canvas)
    Line(((x, y_min), (x, y_max))).write(canvas)
    return I.move_to((x + 0.5 * scale, y))
...
t = I
pins = []
pins.append(objects['+204-TK'].write_power_supply_symbol(canvas, t, '+204-TK'))
t = t.r_move(4.0, 2.0)
pins.append(objects['+000-TK'].write_power_supply_symbol(canvas, t, '+000-TK'))
t = t.r_move(-4.0, 2.0)
pins.append(objects['-B1'].write_power_supply_symbol(canvas, t, '-B1'))
t = t.r_move(4.0, 2.0)
pins.append(objects['-B2'].write_power_supply_symbol(canvas, t, '-B2'))
t = t.r_move(-4.0, 2.0)
pins.append(objects['-B3'].write_power_supply_symbol(canvas, t, '-B3'))
t = t.r_move(4.0, 2.0)
pins.append(objects['-B4'].write_power_supply_symbol(canvas, t, '-B4'))
t = t.r_move(-4.0, 2.0)
pins.append(objects['-B5'].write_power_supply_symbol(canvas, t, '-B5'))
t = t.r_move(4.0, 2.0)
_ = write_multi_join(canvas, list([ ti[1] for ti in pins ]), scale = 2.0)
write_gnd(canvas, _)
_ = write_multi_join(canvas, list([ ti[0] for ti in pins ]), scale = -2.0)
write_ref(canvas, _, '+12V DC')
